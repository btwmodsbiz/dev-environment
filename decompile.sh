...
function main() {
	...
	local branch="$(cd "$SCRIPTDIR/$CLIENT_SRC_PROJECT" 2> /dev/null && git symbolic-ref -q HEAD 2> /dev/null)"
	...
	if $doclient && [ -d "$SCRIPTDIR/$CLIENT_SRC_PROJECT/src" ]; then
		...
		[ $? -ne 0 ] && echo "ERROR: Could not get a list of untracked/ignored files for: $CLIENT_SRC_PROJECT" && EXITCLEAN 1
		
		statuslines="$(grep -v '!! bin/' "$TEMPDIR/gitstatus" | wc -l | tr -d '\t ')"
		[ "$statuslines" != "0" ] && echo "ERROR: One or more untracked or ignored files are in: $CLIENT_SRC_PROJECT" && echo "       You must remove or stash them. See: git status -u --ignored" && EXITCLEAN 1
		
		echo "Removing $CLIENT_SRC_PROJECT/src..."
		rm -rf "$SCRIPTDIR/$CLIENT_SRC_PROJECT/src"
		[ $? -ne 0 ] && echo "ERROR: Could not remove old src directory." && EXITCLEAN 1
	fi
	...
	if $doserver && [ -d "$SCRIPTDIR/$SERVER_SRC_PROJECT/src" ]; then
		...
		[ $? -ne 0 ] && echo "ERROR: Could not get a list of untracked/ignored files for: $SERVER_SRC_PROJECT" && EXITCLEAN 1
		
		statuslines="$(grep -v '!! bin/' "$TEMPDIR/gitstatus" | wc -l | tr -d '\t ')"
		[ "$statuslines" != "0" ] && echo "ERROR: One or more untracked or ignored files are in: $SERVER_SRC_PROJECT" && echo "       You must remove or stash them. See: git status -u --ignored" && EXITCLEAN 1
